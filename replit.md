...
- **Pi AI Matchmaker (Foundation):** Pi is Rhome's AI matchmaker persona. Foundation layer includes: DB tables (`pi_match_insights`, `pi_deck_rankings`, `pi_host_recommendations`, `pi_usage_log`), profile columns (`ideal_roommate_text`, `pi_parsed_preferences`), TypeScript interfaces (`PiMatchInsight`, `PiDeckRanking`, `PiHostRecommendation`, `PiUsageLog`, `PiParsedPreferences`), core service (`services/piMatchingService.ts`) with cache-aware getters, edge function callers, usage tracking and quota enforcement. Plan-gated: renter daily limits (free:5, plus:50, elite:200), agent/company monthly limits tiered by plan. Pi is additive — layers on existing 100-pt deterministic algorithm.
