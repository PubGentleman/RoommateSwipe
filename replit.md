...
- **Role-Specific Features:**
    - **Renter:** Swipe-based matching, 1-on-1 messaging, group management, property exploration with advanced filters and transit integration, saved properties, AI Match Assistant, property reviews, chat scheduling (visit requests + booking offers), and notifications. Features an "Renter Intent System" with split onboarding: place seekers (solo/with_partner/have_group) go through a 4-step flow (budget+room type, neighborhoods, bedrooms+amenities, move-in timeline), while roommate seekers (with_roommates) go through a 4-step flow (budget+room type, dealbreakers, lifestyle, ideal roommate). Both paths collect type-specific data for Best Match scoring from day one. **Duplicate question elimination**: Each field is asked exactly once — signup collects only account type, credentials, location, and photo; type-specific onboarding collects budget, room type, and intent-specific preferences; the profile questionnaire auto-skips steps where data already exists; ApartmentPreferencesScreen is edit-only (pre-populated from existing data, accessible only from Settings).
