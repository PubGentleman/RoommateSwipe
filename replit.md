...
- **Profile Boost System:** Tier-based boost with different durations — Basic: 12 hours ($4.99), Plus: 24 hours (1 free/week, $4.99 for extra at 12h), Elite: 48 hours (unlimited free). `boostData` on User tracks `isBoosted`, `boostExpiresAt` (ISO string), `lastBoostActivatedAt`, `nextFreeBoostAvailableAt`, `boostDurationHours`. Utility functions in `utils/boostUtils.ts`. ProfileScreen has coral gradient "Boost Profile" pill button (gold pulsing when active with countdown). Tier-aware bottom sheet modal for activation. Boosted profiles show subtle coral glow border and small zap badge on swipe cards. Boosted profiles appear in top 20% of swipe deck sorted by compatibility. Expired boosts auto-clear on app foreground with system notification.
