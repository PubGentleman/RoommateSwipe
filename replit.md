...
- **Contact Info Protection:** Platform-level contact info blurring in chat messages. Free users see contact info blurred with "Contact info hidden" lock banner and actionable CTAs ("Book a Showing" / "Upgrade"). Plus/Elite users see contact info freely. All users auto-unlock contact info for a specific conversation once a visit request or booking offer is confirmed/accepted in that thread. No user-facing toggle — this is a platform default computed from plan + booking status.
