...
- **Agent License Verification:** State selector, license number input with helper text, optional document upload (PDF/photo via expo-document-picker), free state licensing board scraping via Supabase Edge Function (`verify-agent-license`) covering 10 states (NY, FL, TX, CA, GA, NC, IL, AZ, NV, CO) with automatic fallback to manual review for unsupported states. Verified/pending agent badges on profiles and listing cards. DB fields: `license_state`, `license_document_url`, `license_verified`, `license_verified_at`, `license_verification_status`. Migration: `034_agent_verification.sql`.
