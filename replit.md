...
**Smart City Selector (Roommates Screen):** The Roommates/Match screen has a city pill button (map-pin icon + city name + dropdown arrow) with a filter icon button beside it. Tapping the city pill opens a bottom sheet modal with Recently Viewed cities (last 3, persisted via AsyncStorage), Popular Cities chips, and a search bar for all cities. Selected city is saved to AsyncStorage for session persistence. If no city is set on first load, a full-screen city picker prompt is shown. Groups and Explore screens retain their horizontal scrollable city pill rows.
